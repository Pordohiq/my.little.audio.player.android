...
- **`get_element_by_Uri(Uri needed_uri, List<DiskElement> data)`** This function finds the element with the Uri and returns null if not found. This method is recursive.
- **`get_musics_at_path(List<String> path, boolean recursive, List<DiskElement> data)`** This function returns a list of all `Music` elements found at the given path. It can be recursive.
...
- **`is_subPath(List<String> list, List<String> prefix)`** This function checks if a list starts with a certain prefix. Used for path comparisons.
...
- **`move_file(DiskElement element, List<String> path)`** This function moves a file to a new path.
